...
class KalmanRegression(OnlineRegression):
    ...
    def get_batch_spread(self, observations):
        return observations[:, 1] - (self.cur_beta * observations[:, 0] + self.cur_alpha)
...
class CointegrationTest(OnlineRegression):
    """
    Tests for cointegration between two time series using the Augmented Dickey-Fuller test in an Online fashion.
    """
    def __init__(self, ts1, ts2, maxlen=50):
        super().__init__(ts1, ts2)
        self.ts1 = deque(ts1, maxlen=maxlen)
        self.ts2 = deque(ts2, maxlen=maxlen)
        self.adf_pvalues = []
    ...
    def run(self):
        data = pd.DataFrame({self._x: list(self.ts1), self._y: list(self.ts2)})
    ...
    def get_recent_spreads(self, window_size):
