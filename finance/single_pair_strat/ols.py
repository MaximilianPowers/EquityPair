from analytics_module.pair_analysis import OLSRegression
...
class OLSStrategy(Strategy):
    ...
    """
    def __init__(self, capital, ticker_1, ticker_2, start_training_date, end_training_date, start_date, end_date, hyperparameters = {}, time_series = None):
        ...
        self.ols = OLSRegression(self.time_series_1, self.time_series_2, 
    ...
    def set_hyperparameters(self, hyperparameters):
        ...
        print(self.hyperparameters["sell_sigma_high"], self.hyperparameters["sell_sigma_low"])
    ...
    def train_model(self):
        self.ols.run()
        tmp =  self.time_series_2 - self.ols.cur_beta*self.time_series_1  - self.ols.cur_alpha
    ...
    def update_threshold(self, observation):
        ...
        spread = self.ols.get_spread(observation) # Calculate spread
        self.n += 1
    ...
    def trade_model(self):
        ...
        for indx, observation in enumerate(observations):
            ...
            self.store_results([
                    ...
                    self.ols.get_spread(observation),
                    self.ols.cur_beta,
                    self.ols.cur_alpha
            ])
            date = dates[indx]
            if self.cur_pos == 0:
                ...
                if buy_1 or buy_2:
                    self.execute_trade(date, self.ols.cur_beta)
            ...
            if portfolio_value < 0:
                print("Portfolio value is negative. Exiting.")
    ...
    def buy_condition(self, observation):
        ...
        if self.ols.get_spread(observation) > self.threshold_normal_buy:
            return True, False
        if self.ols.get_spread(observation) < self.threshold_swapped_buy:
            return False, True
        else:
            return False, False
    ...
    def sell_condition(self, observation):
        ...
        if self.cur_pos == 1:
            if self.ols.get_spread(observation) < self.threshold_normal_sell_low:
                return True
            if self.ols.get_spread(observation) > self.threshold_normal_sell_high:
                return True
        if self.cur_pos == -1:
            if self.ols.get_spread(observation) > self.threshold_swapped_sell_low:
                return True
            if self.ols.get_spread(observation) < self.threshold_swapped_sell_high:
                return True
