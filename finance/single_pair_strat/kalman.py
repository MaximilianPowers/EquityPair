from analytics_module.pair_analysis import KalmanRegression
...
class KalmanStrategy(Strategy):
    ...
    def train_model(self):
        self.kalman.run()
        tmp = self.time_series_2 - self.kalman.cur_beta*self.time_series_1  - self.kalman.cur_alpha
    ...
    def update_threshold(self, observation):
        ...
        spread = self.kalman.get_spread(observation) # Calculate spread
        self.n += 1
    ...
    def trade_model(self):
        ...
        for indx, observation in enumerate(observations):
            ...
            self.store_results([
                    ...
                    self.kalman.get_spread(observation),
                    self.kalman.cur_beta,
                    self.kalman.cur_alpha,
            ])
            date = dates[indx]
            ...
            if self.cur_pos == 0:
                ...
                if buy_1 or buy_2:
                    self.execute_trade(date, self.kalman.cur_beta)
    ...
    def buy_condition(self, observation):
        ...
        if self.kalman.get_spread(observation) > self.threshold_normal_buy:
            return True, False
        if self.kalman.get_spread(observation) < self.threshold_swapped_buy:
            return False, True
        else:
            return False, False
    ...
    def sell_condition(self, observation):
        ...
        if self.cur_pos == 1:
            if self.kalman.get_spread(observation) < self.threshold_normal_sell_low:
                return True
            if self.kalman.get_spread(observation) > self.threshold_normal_sell_high:
                return True
        if self.cur_pos == -1:
            if self.kalman.get_spread(observation) > self.threshold_swapped_sell_low:
                return True
            if self.kalman.get_spread(observation) < self.threshold_swapped_sell_high:
                return True
